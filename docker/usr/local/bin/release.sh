...
collect_core() {
    for machine in $TQEM_MACHINES; do
        ...
        # core-image
        tqem-copy.sh "$TQEM_YOCTO_DEPLOY_IMAGES_PATH/$machine/em-image-core-$machine.tar" "$TQEM_COLLECT_PATH/core-image/$machine" --links

        # bootloader
        archive_bootloaders "$machine"
        tqem-copy.sh "$TQEM_YOCTO_DEPLOY_IMAGES_PATH/$machine/em-image-core-$machine.bootloader.tar" "$TQEM_COLLECT_PATH/core-image/$machine"

        # toolchain
        tqem-copy.sh "$TQEM_YOCTO_DEPLOY_SDK_PATH/emos-x86_64-$architecture-toolchain.sh" "$TQEM_COLLECT_PATH/toolchain/$architecture"
    done
}
